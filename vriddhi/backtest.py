...
Simulates the full agent day by day:
  ...
  2. The Hedge allocator blends strategies using yesterday's per-strategy PnL.
...
def run_backtest(universe: dict[str, pd.DataFrame],
    ...
    # --- gather strategy signals (positions held from t close to t+1 close)
    strat_pos: dict[str, dict[str, pd.Series]] = {}
    for st in RULE_STRATEGIES:
    ...
    for t in range(1, n):
        ...
        strat_rets = []
        for name in names:
            p = np.array([strat_pos[name][s].iloc[t - 1] for s in symbols])
            strat_rets.append(float(np.dot(p, r_t) / len(symbols)))
        ...
        for k, name in enumerate(names):
            p = np.array([strat_pos[name][s].iloc[t - 1] for s in symbols])
            target += w[k] * p
        target = np.clip(target, 0.0, 1.0)  # long-only spot
